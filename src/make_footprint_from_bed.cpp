/*
    Copyright (C) 2009,2010,2011,2012 Stefan Enroth
...
int main(int argc, char* argv[]) 
{
  ...
  string errorLine =  "usage " + 
    ...
    "\t-q <tab separated query file, BED-formatted.>\n" + 
    "\t-e <errorfile, defaults to 'err.txt', failed queries end up here>\n" + 
    "\t-d <offset, how long distance +/- the positions (determined by '-t') to consider. Default:1000>\n" + 
    "\t-t <which position to ccenter the footprints over, 'c' center, 's' start or 'e' end. Start & End taken with respect to strand. Default 'c'.>\n" +
    ...
    "\t-avg <toggle average(mean) of regions. Only applicable to the '-a' results. Default off, overrides '-a'>\n" + 
    "\t-errSymb <writes this for failed queries. Only applicable if '-a' and '-avg' is set. Default 'NA'>\n" + 
    "\t-useOrg <toggle. overrides '-d' and uses original coordinates/regions instead. Only applicable when -a (and -avg) are set. Default Off>\n" +
	"\t-nrBins <if given, partions the given interval into 'nrBins' sections and averages values inside, adds -d bases before and after. \n" +
	"\t         If each section is less than 1bp 'errSymb' is written. As far as possible, the secions will be of similar sizes in bp. \n" + 
	"\t         Automatically turns '-useOrg' off.> \n" + 
	" Writes 4 lines in each outfile (not 'all_'-files) \n" + 
  ...
  for (int i=1;i<argc;i++)
    {
      ...
      else if(strcmp(argv[i],"-useOrg") == 0)
		useOrg = 1;
      else if(strcmp(argv[i],"-nrBins") == 0)
		nrBins = atoi(argv[++i]);
      else
		{
  ...
  int nq = queryControlBED(&infq,&seqMap,true,type);
  ...
  if(getMultipleSignalsFPfromBED(&infq,seqFiles,outfile,efile,
								 offset,progress,co,move,both,nq,all,type,avg,errSymb,useOrg,nrBins)<0)
    {
